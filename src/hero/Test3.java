...
class gameFrame extends JFrame implements KeyListener, Runnable {
	...
	public void init() {
		...
		Player_img = new ImageIcon("Image/gamege/eggsmall.png").getImage();
		BackGround_img = new ImageIcon("Image/gamege/firstbackground.jpg").getImage();
		BackGround2_img = new ImageIcon("Image/gamege/firstbackground2.jpg").getImage();
		Boss1_img = new ImageIcon("Image/gamege/boss.png").getImage();
	...
	public void MissileProcess() {
		...
		for (int i = 0; i < Missile_List.size(); i++) {
			...
			if (Crash(x, y, ms.x, ms.y, Player_img, Missile2_img) && (ms.who == 2 || ms.who == 4)) {
				player_Hitpoint -= 3;
				if (player_Hitpoint < 1) {
					Player_life = 0;
				}
	...
	public void BossProcess() {
		...
		for (int i = 0; i < Boss_List.size(); i++) {
			...
			if (cnt % 15 == 0) {
				ms = new Missile(bs.x + 50, bs.y + 200, missile_Speed, 1, 180);
				Missile_List.add(ms);
			}
			if (cnt % 30 == 0) {
				ms = new Missile(bs.x + 50, bs.y + 200, missile_Speed, 2, 165);
				Missile_List.add(ms);
				ms = new Missile(bs.x + 50, bs.y + 200, missile_Speed, 2, 195);
				Missile_List.add(ms);
			}
			if (cnt % 60 == 0) {
				ms = new Missile(bs.x + 50, bs.y + 200, missile_Speed, 2, 150);
				Missile_List.add(ms);
				ms = new Missile(bs.x + 50, bs.y + 200, missile_Speed, 2, 210);
				Missile_List.add(ms);
			}
	...
	public void Draw_Missile() {
		for (int i = 0; i < Missile_List.size(); i++) {
			...
			if (ms.who == 0) {
				...
				buffg.drawImage(Missile_img, ms.x, ms.y, this);
			} else if (ms.who == 2 || ms.who == 3 || ms.who == 4) {
...
class Missile {
	...
	Missile(int x, int y, int speed, int who, int angle) {
		this.x = x;
		this.y = y;
		this.speed = speed;
		this.who = who;
...
class BossExplo {
	int x; // 이미지를 그릴 x 좌표
	int y; // 이미지를 그릴 y 좌표
	int bex_cnt; // 이미지를 순차적으로 그리기 위한 카운터

	BossExplo(int x, int y) {
		this.x = x;
		this.y = y;
		bex_cnt = 0;
	}
	...
	public void effect() {
		bex_cnt++; // 해당 메소드 호출 시 카운터를 +1 시킨다.
	}
}