...
public class Test2 {
	...
	public static void main(String[] args) {
		...
		gameFrame2 fms = new gameFrame2();
	}

}

class gameFrame2 extends JFrame implements KeyListener, Runnable {
	...
	public void init() {
		...
		Missile_2img = new ImageIcon("Image/gamege/EneM3.png").getImage();
		Missile2_img = new ImageIcon("Image/gamege/Missile2_img.png").getImage();
		Enemy_img = new ImageIcon("Image/gamege/ppapsmall.png").getImage();
		Enemy2_img = new ImageIcon("Image/gamege/Enemy3.png").getImage();
		Player_img = new ImageIcon("Image/gamege/goldsmall.png").getImage();
		BackGround_img = new ImageIcon("Image/gamege/lastbackground.jpg").getImage();
		BackGround2_img = new ImageIcon("Image/gamege/lastbackground2.jpg").getImage();
		Boss1_img = new ImageIcon("Image/gamege/bossEE.png").getImage();
		GameOver = new ImageIcon("Image/gamege/GameOver.png").getImage();
		GameWin = new ImageIcon("Image/gamege/GameWin.png").getImage();
		...
		for (int i = 0; i < Cloud_img.length; i++) {
			Cloud_img[i] = new ImageIcon("Image/gamege/cloud_" + i + ".png").getImage();
		}
			...
			while (true) {
				if (gamestart == 1) {
					...
					if (Boss_life == 1) {
						...
						Boss_Delay++;
						/*
						 * if(Boss_Delay==50) { Boss_life=1; Boss_Delay=0; bsHP=bsHP2; }
						 */
					}
					cnt++;
					time++;
	...
	public void MissileProcess() {
		...
		for (int i = 0; i < Missile_List.size(); i++) {
			...
			for (int k = 0; k < Enemy2_List.size(); k++) {
				e2 = (Enemy2) Enemy2_List.get(k);
				if (Crash(ms.x, ms.y, e2.x, e2.y, Missile_img, Enemy2_img) && ms.who == 0) {
					EnHP2--;
					Missile_List.remove(i);
					if (EnHP2 == 0) {
						Enemy2_List.remove(k);
						game_Score += 80;
						ex = new Explosion(e2.x + Enemy2_img.getWidth(null) / 2, e2.y + Enemy2_img.getHeight(null) / 2,
								0);
						Explosion_List.add(ex);
						EnHP2 = 30;
					}
	...
	public void EnemyProcess() {
		for (int i = 0; i < Enemy_List.size(); i++) {
			...
			if (cnt % 30 == 0) {
				ms = new Missile(en.x, en.y + 25, missile_Speed, 1, 180);
				Missile_List.add(ms);
			}
		...
		if (cnt % bstime == bstime - 10) {
			e2 = new Enemy2(f_width + 100, 100, enemy_speed);
			Enemy2_List.add(e2);
			e2 = new Enemy2(f_width + 100, 300, enemy_speed);
			Enemy2_List.add(e2);
			e2 = new Enemy2(f_width + 100, 500, enemy_speed);
			Enemy2_List.add(e2);
	...
	public void EnemyProcess2() {
		for (int i = 0; i < Enemy2_List.size(); i++) {
			e2 = (Enemy2) (Enemy2_List.get(i));
			e2.move();

			if (cnt % 30 == 0) {
				ms = new Missile(e2.x, e2.y + 25, missile_Speed, 4, 210);
				Missile_List.add(ms);
			}
			if (e2.x < -200) {
				Enemy2_List.remove(i);
			}
			if (Crash(x, y, e2.x, e2.y, Player_img, Enemy2_img)) {
				player_Hitpoint--; // 플레이어 체력을 1깍습니다.
				if (player_Hitpoint < 1) {
					Player_life = 0;
				}
				Enemy2_List.remove(i); // 적을 제거합니다.
				game_Score += 10;
				ex = new Explosion(e2.x + Enemy2_img.getWidth(null) / 2, e2.y + Enemy2_img.getHeight(null) / 2, 0);
				Explosion_List.add(ex);
				ex = new Explosion(x, y, 1);
				Explosion_List.add(ex);
			}
		}
		if (cnt % bstime == cnt % bstime - 10) {
			e2 = new Enemy2(f_width + 100, 200, enemy_speed);
			Enemy2_List.add(e2);
			e2 = new Enemy2(f_width + 100, 400, enemy_speed);
			Enemy2_List.add(e2);
		}
	}
	...
	public void BossProcess() {
		...
		for (int i = 0; i < Boss_List.size(); i++) {
			...
			if (cnt % 15 == 0) {
				ms = new Missile(bs.x + 50, bs.y + 210, missile_Speed, 3, 180);
				Missile_List.add(ms);
			}
			if (cnt % 30 == 0) {
	...
	public void Draw_Background() {
		...
		if (bx > -4188) {
			...
			buffg.drawImage(BackGround2_img, bx + 2094, 0, this);
			buffg.drawImage(BackGround_img, bx + 4188, 0, this);
			bx -= 1;
		} else {
			bx = 0;
		}
	...
	public void Draw_Missile() {
		for (int i = 0; i < Missile_List.size(); i++) {
			...
			if (ms.who == 0) {
				...
				buffg.drawImage(Missile_2img, ms.x, ms.y, this);
	...
	public void Draw_Enemy2() {
		for (int i = 0; i < Enemy2_List.size(); i++) {
			e2 = (Enemy2) (Enemy2_List.get(i));
			buffg.drawImage(Enemy2_img, e2.x, e2.y, this);
		}
	}
	...
	public void Draw_StatusText() { // 상태 체크용 텍스트를 그립니다.
		...
		if (Boss_life == 0) {
			buffg.drawImage(GameWin, 260, 167, this);
		}
	}
